/**
 * TLDR Shield — Mixtral-8x22b Solo Deep Scan Validation
 * ======================================================
 * Single model (mixtral-8x22b) with refined prompt.
 * 5 runs per service = 25 total API calls for statistical validation.
 * Also runs Qwen3-80b solo for direct A/B comparison.
 */
...
if (NIM_KEYS.length === 0) { console.error("❌ No NIM API keys found."); process.exit(1); }
...
async function nimCreateWithRetry(params: any, signal: AbortSignal) {
  ...
  for (let attempt = 0; attempt < NIM_KEYS.length; attempt++) {
    ...
    const kt = setTimeout(() => kc.abort(), PER_KEY_TIMEOUT_MS);
    const onAbort = () => kc.abort();
    signal.addEventListener("abort", onAbort, { once: true });
    try {
      return await client.chat.completions.create(params, { signal: kc.signal });
    } catch (err: any) {
      lastError = err;
      const s = err?.status ?? 0;
      if (s >= 400 && s < 500 && s !== 429) throw err;
      if (signal.aborted) throw err;
      console.warn(`  ⚠ Key #${attempt + 1} failed (status=${s})`);
    } finally { clearTimeout(kt); signal.removeEventListener("abort", onAbort); }
  }
  return null;
}
...
function extractJSON(text: string): any | null {
  ...
  const normalize = (s: string) => s.replace(/,\s*([}\]])/g, "$1");
  ...
  for (const src of [clean(text), text]) {
    ...
    const p = tryParse(c) ?? tryParse(normalize(c));
    if (p && typeof p === "object") {
      if (p.pillars) for (const k of Object.keys(p.pillars)) {
        const v = p.pillars[k];
        if (v && typeof v.violation === "string") v.violation = v.violation.toLowerCase() === "true";
      }
      return p;
    }
  }
...
interface GroundTruth {
  service: string; text: string; expectedRating: string[]; scoreRange: [number, number];
...
async function main() {
  console.log("╔════════════════════════════════════════════════════════════╗");
  console.log("║  SOLO MODEL VALIDATION: 2 models × 5 services × 5 runs  ║");
  console.log("║  Refined prompt — targeting >98% pillar accuracy         ║");
  console.log("╚════════════════════════════════════════════════════════════╝\n");

  let md = "# Deep Scan Solo Model Validation\n\n";
  md += `> **Models:** Mixtral-8x22b + Qwen3-80b (head-to-head)\n`;
  md += `> **Runs:** 5 per service per model = 50 total API calls\n`;
  md += `> **Prompt:** Refined v3 (fixed data_retention, ai_training, transparency)\n\n`;

  const stats: Record<string, {
    parseOk: number; total: number; ratingOk: number; scoreOk: number;
    pillarCorrect: number; pillarTotal: number; latencies: number[];
    misses: Record<string, number>;
  }> = {};

  for (const m of MODELS) stats[m] = { parseOk: 0, total: 0, ratingOk: 0, scoreOk: 0, pillarCorrect: 0, pillarTotal: 0, latencies: [], misses: {} };

  for (const tc of TEST_CASES) {
    const pad = Math.floor((55 - tc.service.length) / 2);
    md += "=======================================================\n";
    md += `${" ".repeat(Math.max(0, pad))}${tc.service}\n`;
    md += `Expected: ${tc.expectedRating.join("/")} | Score: ${tc.scoreRange[0]}-${tc.scoreRange[1]}\n`;
    md += `Ground Truth: ${PILLAR_KEYS.filter(k => tc.expectedPillars[k]).join(", ") || "None"}\n`;
    md += "-------------------------------------------------------\n";

    for (const model of MODELS) {
      const shortName = model.includes("mixtral") ? "Mixtral-8x22b" : "Qwen3-80b";
      md += `\n### ${shortName}\n`;

      for (let run = 1; run <= RUNS; run++) {
        console.log(`${tc.service} → ${shortName} (run ${run}/${RUNS})`);
        ...
        if (r.parsed) {
          s.parseOk++;
          const violations = PILLAR_KEYS.filter(k => r.parsed.pillars?.[k]?.violation).join(", ");
          const ratingOk = tc.expectedRating.includes(r.parsed.rating);
          const scoreOk = r.parsed.score >= tc.scoreRange[0] && r.parsed.score <= tc.scoreRange[1];
          if (ratingOk) s.ratingOk++;
          if (scoreOk) s.scoreOk++;
          s.latencies.push(r.latencyMs);

          const pillarMisses: string[] = [];
          for (const k of PILLAR_KEYS) {
            s.pillarTotal++;
            const got = Boolean(r.parsed.pillars?.[k]?.violation);
            if (got === tc.expectedPillars[k]) {
              s.pillarCorrect++;
            } else {
              pillarMisses.push(`${k}(exp=${tc.expectedPillars[k]},got=${got})`);
              s.misses[`${tc.service}:${k}`] = (s.misses[`${tc.service}:${k}`] || 0) + 1;
            }
          }

          const latFlag = r.latencyMs > 20000 ? " ⚠️" : " ✅";
          md += `Run ${run}: ${r.parsed.rating} ${ratingOk ? "✅" : "❌"}, Score: ${r.parsed.score} ${scoreOk ? "✅" : "❌"}, Violations: ${violations || "None"} [${(r.latencyMs/1000).toFixed(2)}s${latFlag}]`;
          if (pillarMisses.length > 0) md += ` | MISS: ${pillarMisses.join(", ")}`;
          else md += ` | Pillars: 5/5 ✅`;
          md += "\n";
        } else {
  ...
  // Summary
  md += "\n=======================================================\n";
  md += "              FINAL VALIDATION SUMMARY\n";
  md += "=======================================================\n\n";

  md += "## Head-to-Head Comparison\n\n";
  md += "| Metric | Mixtral-8x22b | Qwen3-80b |\n";
  md += "|--------|--------------|----------|\n";

  for (const model of MODELS) {
    const s = stats[model];
    const shortName = model.includes("mixtral") ? "Mixtral-8x22b" : "Qwen3-80b";
    // will build row by row below
  }

  const mx = stats[MODELS[0]];
  const qw = stats[MODELS[1]];

  md += `| Parse Rate | ${mx.parseOk}/${mx.total} (${mx.total > 0 ? ((mx.parseOk/mx.total)*100).toFixed(0) : 0}%) | ${qw.parseOk}/${qw.total} (${qw.total > 0 ? ((qw.parseOk/qw.total)*100).toFixed(0) : 0}%) |\n`;
  md += `| Rating Accuracy | ${mx.ratingOk}/${mx.parseOk} (${mx.parseOk > 0 ? ((mx.ratingOk/mx.parseOk)*100).toFixed(1) : 0}%) | ${qw.ratingOk}/${qw.parseOk} (${qw.parseOk > 0 ? ((qw.ratingOk/qw.parseOk)*100).toFixed(1) : 0}%) |\n`;
  md += `| Score in Range | ${mx.scoreOk}/${mx.parseOk} (${mx.parseOk > 0 ? ((mx.scoreOk/mx.parseOk)*100).toFixed(1) : 0}%) | ${qw.scoreOk}/${qw.parseOk} (${qw.parseOk > 0 ? ((qw.scoreOk/qw.parseOk)*100).toFixed(1) : 0}%) |\n`;
  md += `| **Pillar Accuracy** | **${mx.pillarCorrect}/${mx.pillarTotal} (${mx.pillarTotal > 0 ? ((mx.pillarCorrect/mx.pillarTotal)*100).toFixed(1) : 0}%)** | **${qw.pillarCorrect}/${qw.pillarTotal} (${qw.pillarTotal > 0 ? ((qw.pillarCorrect/qw.pillarTotal)*100).toFixed(1) : 0}%)** |\n`;

  if (mx.latencies.length > 0) {
    const mxAvg = mx.latencies.reduce((a, b) => a + b, 0) / mx.latencies.length;
    const qwAvg = qw.latencies.length > 0 ? qw.latencies.reduce((a, b) => a + b, 0) / qw.latencies.length : 0;
    mx.latencies.sort((a, b) => a - b);
    qw.latencies.sort((a, b) => a - b);
    const mxP90 = mx.latencies[Math.floor(mx.latencies.length * 0.9)];
    const qwP90 = qw.latencies.length > 0 ? qw.latencies[Math.floor(qw.latencies.length * 0.9)] : 0;
    md += `| Avg Latency | ${(mxAvg/1000).toFixed(2)}s | ${(qwAvg/1000).toFixed(2)}s |\n`;
    md += `| P90 Latency | ${(mxP90/1000).toFixed(2)}s | ${(qwP90/1000).toFixed(2)}s |\n`;
    md += `| Max Latency | ${(Math.max(...mx.latencies)/1000).toFixed(2)}s | ${(qw.latencies.length > 0 ? Math.max(...qw.latencies)/1000 : 0).toFixed(2)}s |\n`;
    md += `| Under 20s | ${mx.latencies.filter(l => l <= 20000).length}/${mx.latencies.length} | ${qw.latencies.filter(l => l <= 20000).length}/${qw.latencies.length} |\n`;
  }

  // Miss breakdown
  md += "\n## Miss Breakdown (per-pillar per-service)\n\n";
  md += "| Service:Pillar | Mixtral Misses | Qwen Misses |\n";
  md += "|---------------|---------------|-------------|\n";
  const allMissKeys = new Set([...Object.keys(mx.misses), ...Object.keys(qw.misses)]);
  for (const key of [...allMissKeys].sort()) {
    md += `| ${key} | ${mx.misses[key] || 0}/${RUNS} | ${qw.misses[key] || 0}/${RUNS} |\n`;
  }
