...
| Eval suite (`runEval.ts` + `golden.test.ts`) | Working — 30 dataset cases, 10 golden tests + `dataset_basic.jsonl` (20) + `dataset_deep.jsonl` (20) |
...
| GDPR "Right to Erasure" email generator | Working (`POST /api/gdpr-email`, side panel UI) |
| Privacy benchmarking (industry category comparison) | Working (`GET /api/benchmark/:category`, `eval/benchmarks.json`) |
| Batch scan (all legal links on a page) | Working (popup + background queue) |
| NIM key health monitoring | Working (GitHub Actions scheduled workflow + alerts) |
| Policy change notifications | Working (watch URLs, recheck engine, dashboard bell, extension badge) |
| WASM local inference (offline quick scans) | Working (Transformers.js in offscreen document) |
| Firefox MV3 support | Working (manifest overrides + cross-browser shim) |
| Citation confidence scoring | Working (HIGH/MEDIUM/LOW per pillar, shown in popup) |
| Pillar breakdown card in popup | Working (C1/C2 — confidence dots + truncated citations) |
| Profile-based LLM provider switching | Working (switch between NIM models via profile) |
...
- ~~No NIM key health monitoring (key failures are reactive, not proactive)~~ ✅ Proactive health monitoring live (`.github/workflows/nim-health.yml`)
- ~~No user-facing policy change notifications (manual re-scan only)~~ ✅ Policy watch + recheck engine shipped
...
- **Chrome Web Store submission** — extension not yet published; all features complete, store listing not created
- **eval accuracy ceiling** — deep20 dataset: 80% rating accuracy on complex multi-violation policies (Instagram, YouTube, Amazon, Uber underdetected by LLM); quick20: 85%
...
NIM_RETRY_ROUNDS=3            # Default: 3 retry rounds across all keys
NIM_RETRY_BASE_BACKOFF_MS=1200 # Default: 1.2s base backoff (exponential)
NIM_MODEL_QUICK=meta/llama-3.3-70b-instruct  # Default model for quick tier
NIM_MODEL_DEEP=meta/llama-3.3-70b-instruct   # Default model for deep tier
...
| `transparency` | Self-contradictory language within the same policy | Actively self-contradictory only — vague but consistent = LOW severity |
...
**Per-pillar penalty (HIGH/MEDIUM confidence → full, LOW confidence → half):**
| Pillar | Full penalty | LOW penalty | Notes |
|--------|------------|------------|-------|
| `ai_training` | −30 | −15 | |
| `data_selling` | −30 | −15 | |
| `data_retention` | −30 | −15 | |
| `content_ownership` | −30 | −15 | |
| `dark_patterns` | **−40** | **−20** | Elevated — removes legal rights |
| `transparency` | **−20** | **−5** | Self-contradictory = full; merely vague = LOW |

**Scoring bands (score is single source of truth — enforced by `calculateScoreAndRating`, not LLM):**
- 0 violations → 90–100, **SAFE**
- 1 standard violation (−30) → 70, **OKAY**
- 1 dark_patterns violation (−40) → 60, **OKAY**
- 2 standard violations → 40, **RISKY**
- 1 dark_patterns + 1 other → 30, **RISKY**
- 3+ violations → ≤10, **RISKY**

Score < 50 → **RISKY**. Score 50–89 → **OKAY**. Score ≥ 90 → **SAFE**.

**Pillar logic is distributed across FIVE locations that must stay in sync:**
1. `buildSystemPrompt()` — LLM instructions (server.ts ~1615)
2. `applyConsistencyCrossCheck()` — keyword-based secondary check (server.ts ~1014)
3. `buildDeterministicDeepFallback()` — regex fallback when LLM fails (server.ts ~1500)
4. `PILLAR_PENALTY` in `calculateScoreAndRating()` — penalty table (server.ts ~972)
5. `PILLAR_PENALTY` in `eval/runEval.ts` — eval harness mirror of #4
